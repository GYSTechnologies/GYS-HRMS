...
const HRPayrollManagement = () => {
  ...
  return (
    <div className="min-h-[80vh] p-2 sm:p-3">
      {" "}
      {/* Reduced padding */}
      <style>{`
    ...
    `}</style>
      ...
      <header className="mb-3 sm:mb-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
          ...
          <button
            ...
            className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-md text-white primary-btn text-xs sm:text-sm"
      ...
      {/* MAIN CONTENT LAYOUT - Reduced spacing */}
      <div className="flex flex-col xl:flex-row gap-3 sm:gap-4 mb-3 sm:mb-4">
        {/* Left: Stats Cards - Reduced sizes */}
        <div className="flex-shrink-0">
          <div
            className="stats-grid grid grid-cols-2 xl:grid-cols-4 gap-2 sm:gap-3"
            id="stats-cards"
          >
            <div className="stat-card bg-white p-2 sm:p-3 rounded-lg shadow border-l-4 border-blue-500 text-center min-w-[10rem]">
              <h3 className="text-xs text-gray-500">Total Payrolls</h3>
              <p className="text-lg sm:text-xl font-bold">{payrolls.length}</p>
            </div>
            <div className="stat-card bg-white p-2 sm:p-3 rounded-lg shadow border-l-4 border-yellow-500 text-center min-w-[10rem]">
            ...
            <div className="stat-card bg-white p-2 sm:p-3 rounded-lg shadow border-l-4 border-blue-500 text-center min-w-[10rem]">
              <h3 className="text-xs text-gray-500">Pending Approval</h3>
              <p className="text-lg sm:text-xl font-bold text-blue-600">
                {payrolls.filter((p) => p.status === "pending_approval").length}
              </p>
            </div>
            <div className="stat-card bg-white p-2 sm:p-3 rounded-lg shadow border-l-4 border-green-500 text-center min-w-[10rem]">
        ...
        {/* Right: Filters Panel - Reduced sizes */}
        <div
          className="flex-1 bg-white shadow rounded-lg border-l-4 border-blue-500 flex flex-col"
          style={{ minHeight: "70px" }}
        >
          <div className="flex-1 p-2 sm:p-3">
            <div className="filters-grid grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-1.5 sm:gap-2 h-full items-center">
              {/* Year */}
              <div>
                ...
                <select
                  ...
                  className="w-full p-1 text-xs border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
                  {years.map((year) => (
              ...
              {/* Buttons */}
              <div className="filter-buttons flex gap-1 justify-center mt-1 sm:mt-2">
                <button
                  onClick={clearFilters}
                  className="px-2 py-1 text-xs bg-gray-100 text-gray-600 rounded-md hover:bg-gray-200 transition-colors flex items-center gap-1 border"
                  title="Clear Filters"
                >
                  <svg
      ...
      {/* PAYROLL RECORDS TABLE - Reduced sizes */}
      <div className="bg-white shadow rounded-lg overflow-hidden">
        ...
        {payrolls.length === 0 ? (
          <div className="text-center py-6 sm:py-8">
            <svg
              className="w-10 sm:w-12 h-10 sm:h-12 text-gray-300 mx-auto mb-2"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <>
            {/* Desktop Table View - Reduced sizes */}
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {payrolls.map((payroll) => (
                    <tr key={payroll._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 sm:px-4 py-2 whitespace-nowrap text-xs text-gray-900">
                        {formatCurrency(payroll.basic)}
                      </td>
                      <td className="px-3 sm:px-4 py-2 whitespace-nowrap">
                        {getStatusBadge(payroll.status)}
                      </td>
                      <td className="px-3 sm:px-4 py-2 whitespace-nowrap text-xs font-medium">
              ...
              {payrolls.map((payroll) => (
                <div key={payroll._id} className="mobile-table-card">
                  ...
                  <div className="mobile-row">
                    ...
                    <div className="mobile-value flex items-center gap-2 justify-end">
                      <img
