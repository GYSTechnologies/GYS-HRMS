...
const AdminReportsTab = ({ token, addNotification }) => {
  ...
  <div className="">
    ...
    <div className="flex flex-col mb-4 md:mb-6 gap-3">
      <div>
        <h3 className="text-lg sm:text-xl  font-bold text-gray-800">Monthly Attendance Reports</h3>
    ...
    <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-3 md:p-4 mb-4 md:mb-6">
      <h3 className="text-sm md:text-base font-semibold text-gray-800 mb-3">Filter Reports</h3>
        ...
        <div className="space-y-3 md:space-y-0 md:grid md:grid-cols-4 md:gap-3">
          <div>
            ...
            <select
              ...
              className="w-full p-2 text-xs border border-slate-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            >
          ...
          <div className="flex items-end pt-1">
            <button
              ...
              className="w-full px-3 py-2 text-white font-medium text-xs rounded-md transition-all duration-200 shadow-sm"
    ...
    {/* Loading State */}
    {loading ? (
      <div className="text-center py-6 bg-white rounded-lg shadow-sm border border-slate-200">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500 mx-auto mb-2"></div>
        <p className="text-xs text-gray-600 font-medium">Generating reports...</p>
        <p className="text-xs text-gray-500 mt-1">Please wait while we process your data</p>
      </div>
    ) : reports.length === 0 ? (
      /* Empty State */
      <div className="text-center py-8 bg-white rounded-lg shadow-sm border border-slate-200">
        ...
        <h3 className="text-sm font-semibold text-gray-700 mb-1">No report data available</h3>
        <p className="text-xs text-gray-500 max-w-md mx-auto px-2">Try adjusting your filters or select a different date range to generate attendance reports.</p>
      </div>
    ) : (
      /* Reports Table */
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="block lg:hidden space-y-3 p-3">
          {reports.map((report, index) => (
            <div key={index} className="border border-slate-200 rounded-lg p-3 shadow-sm hover:shadow-md transition-shadow">
              {/* Employee Info */}
              <div className="flex items-start justify-between mb-2">
                <div className="flex-1">
                  <h3 className="font-semibold text-gray-900 text-xs truncate">{report.employeeName || "N/A"}</h3>
              ...
              <div className="grid grid-cols-2 gap-2 mb-2">
                <div className="text-center p-1.5 bg-green-50 rounded">
                  ...
                  <p className="font-semibold text-green-600 text-xs">{report.present || 0}</p>
                </div>
                <div className="text-center p-1.5 bg-red-50 rounded">
                  <p className="text-xs text-gray-500">Absent</p>
                  <p className="font-semibold text-red-600 text-xs">{report.absent || 0}</p>
                </div>
                <div className="text-center p-1.5 bg-yellow-50 rounded">
                  <p className="text-xs text-gray-500">Pending</p>
                  <p className="font-semibold text-yellow-600 text-xs">{report.pending || 0}</p>
                </div>
                <div className="text-center p-1.5 bg-red-50 rounded">
                  <p className="text-xs text-gray-500">Rejected</p>
                  <p className="font-semibold text-red-600 text-xs">{report.rejected || 0}</p>
                </div>
              </div>
              ...
              <div className="flex justify-between items-center pt-2 border-t border-slate-100">
                <div>
                  ...
                  <p className="font-semibold text-gray-800 text-xs">{report.totalDays || 0}</p>
                </div>
            ...
            <table className="w-full border-collapse min-w-[800px]">
              <thead className="bg-[#104774] text-white sticky top-0">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left text-xs font-semibold uppercase">Employee ID</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold uppercase">Department</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold uppercase">Present</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold uppercase">Absent</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold uppercase">Pending</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold uppercase">Rejected</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold uppercase">Total Days</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold uppercase">Attendance %</th>
                </tr>
              </thead>
                ...
                {reports.map((report, index) => (
                  <tr key={index} className="hover:bg-slate-50 transition-colors">
                    <td className="px-3 py-2">
                      <div className="font-medium text-gray-900 text-xs">{report.employeeName || "N/A"}</div>
                      <div className="text-xs text-gray-500">{report.employeeEmail || "N/A"}</div>
                    </td>
                    <td className="px-3 py-2 text-xs text-gray-700">{report.employeeId || "N/A"}</td>
                    <td className="px-3 py-2">
                      <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs">
                        {report.department || "N/A"}
                      </span>
                    </td>
                    <td className="px-3 py-2 text-green-600 font-semibold text-xs">{report.present || 0}</td>
                    <td className="px-3 py-2 text-red-600 font-semibold text-xs">{report.absent || 0}</td>
                    <td className="px-3 py-2 text-yellow-600 font-semibold text-xs">{report.pending || 0}</td>
                    <td className="px-3 py-2 text-red-600 font-semibold text-xs">{report.rejected || 0}</td>
                    <td className="px-3 py-2 font-semibold text-gray-700 text-xs">{report.totalDays || 0}</td>
                    <td className="px-3 py-2">
