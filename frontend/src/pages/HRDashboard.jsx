  ...
  const handleApproveAttendance = async (attendanceId) => {
    try {
      await axiosInstance.patch(
        `/dashboard/hr/approve-attendance/${attendanceId}`,
        { status: "approved" }
      );
    ...
    <div className="max-h-screen bg-gray-50 ">
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <button
          ...
          disabled={refreshing}
          className="bg-[#104774] text-white px-3 py-2 rounded-md flex items-center justify-center disabled:opacity-50 w-full sm:w-auto text-sm"
        >
          <RefreshCw
            className={`w-3 h-3 mr-2 ${refreshing ? "animate-spin" : ""}`}
          />
      ...
      <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4 mb-6">
        ...
        <div
          onClick={() => {
            ...
            else if (hrAttendance.checkIn && !hrAttendance.checkOut)
              handleLogout();
          }}
          className="bg-gradient-to-r from-yellow-100 to-yellow-200 rounded-xl p-3 shadow-md hover:shadow-lg transition-all duration-200 border border-slate-200 cursor-pointer"
        >
          <div className="mb-2 flex items-center justify-between">
            ...
            >
              {hrAttendance.status
                ? hrAttendance.status.charAt(0).toUpperCase() +
                  hrAttendance.status.slice(1)
                : "Not marked"}
            </span>
          </div>
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium text-slate-900">
                {hrAttendance.checkIn
                  ? new Date(hrAttendance.checkIn).toLocaleTimeString()
                  : "Not logged in"}
              </span>
            </div>
            <div className="flex justify-between">
              ...
              <span className="font-medium text-slate-900">
                {hrAttendance.checkOut
                  ? new Date(hrAttendance.checkOut).toLocaleTimeString()
                  : "Pending"}
              </span>
            </div>
          </div>

          <div className="text-center text-xs text-slate-700 mt-2 font-medium">
            {hrAttendance.checkOut
        ...
        <div
          ...
          className="bg-gradient-to-r from-blue-100 to-blue-200 rounded-xl p-3 shadow-md hover:shadow-lg transition-all duration-200 border border-slate-200 cursor-pointer"
        >
        ...
        <div
          ...
          className="bg-gradient-to-r from-orange-100 to-orange-200 rounded-xl p-3 shadow-md hover:shadow-lg transition-all duration-200 border border-slate-200 cursor-pointer"
        >
        ...
        <div
          ...
          className="bg-gradient-to-r from-green-100 to-green-200 rounded-xl p-3 shadow-md hover:shadow-lg transition-all duration-200 border border-slate-200 cursor-pointer"
        >
          <h3 className="font-semibold text-slate-800 text-sm mb-1">
            Payroll Processed
          </h3>
          <div className="text-xl font-bold text-slate-800 mb-1">
            {dashboardData?.processedPayroll || 0}/
            {dashboardData?.totalEmployees || 0}
          </div>
        ...
        {/* New Joiners */}
        <div className="bg-gradient-to-r from-purple-100 to-purple-200 rounded-xl p-3 shadow-md hover:shadow-lg transition-all duration-200 border border-slate-200 cursor-pointer">
          <h3 className="font-semibold text-slate-800 text-sm mb-1">
            New Joiners
          </h3>
          <div className="text-xl font-bold text-slate-800 mb-1">
      ...
      {/* Attendance Table - More Compact */}
      <div className="bg-white rounded-xl p-4 shadow-md hover:shadow-lg transition-all duration-200 border border-slate-200">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-semibold text-slate-800">Log in/Out</h2>
        ...
        {/* Desktop Table View - More Compact */}
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {attendanceLog.map((log, index) => (
                ...
                >
                  <td className="px-3 py-2 text-sm text-slate-800 max-w-[120px] truncate">
                    {log.name}
                  </td>
                  ...
                  <td className="px-3 py-2 text-sm text-slate-600 max-w-[120px] truncate">
                    {log.task || "-"}
                  </td>
                  <td className="px-3 py-2 text-sm text-slate-600 max-w-[120px] truncate">
                    {log.notes || "-"}
