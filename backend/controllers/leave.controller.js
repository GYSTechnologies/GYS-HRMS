...
// /**
//  * Get remaining leaves for a given user and year.
//  * Returns an object { casual: n, sick: n, paid: n } and the policy used.
//  */
...
//   // fetch approved leaves that overlap with the year
...
//   try {
//     const { role, year, casual, sick, paid } = req.body;
//     if (!role || !year) return res.status(400).json({ message: "role and year are required" });

//     let policy = await LeavePolicy.findOne({ role, year });
//     if (policy) {
//       policy.leaves.casual = casual ?? policy.leaves.casual;
//       policy.leaves.sick = sick ?? policy.leaves.sick;
//       policy.leaves.paid = paid ?? policy.leaves.paid;
//       await policy.save();
//     } else {
//       policy = await LeavePolicy.create({
//         role,
//         year,
//         leaves: { casual: casual ?? 0, sick: sick ?? 0, paid: paid ?? 0 },
//       });
//     }
...
//     }).lean(); // 👈 lean() use karenge taaki plain JS object mile

//     if (!policy) {
//       return res.status(404).json({ message: "Leave policy not set" });
//     }

//     // Get approved leaves of user
//     const approvedLeaves = await Leave.find({ user: user._id, status: "approved" });
...
//     // Calculate balance in plain JS object
//     const balance = {};
...
//   return { policy, remaining };
// }
// In leave.controller.js - update computeRemainingForUserAndYear function
async function computeRemainingForUserAndYear(userId, year) {
  const user = await User.findById(userId);
  if (!user) throw new Error("User not found");
...
// In leave.controller.js - update setLeavePolicy
export const setLeavePolicy = async (req, res) => {
  try {
